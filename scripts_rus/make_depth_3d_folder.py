...
def extract_frames(start_frame, end_frame):
    ...
    # Список файлов исходя из размера чанка
    chunk_files = all_frames[start_frame:end_frame+1]  # end_frame включительно
    print(f"\n-- ФРЕЙМЫ НОВОГО ПОТОКА --\nФреймы {start_frame} - {end_frame}\n")
...
def run_processing():
    ''' Главная функция запуска потоков обработки '''
    ...
    if isinstance(total_frames, int):
        with ThreadPoolExecutor(max_workers=max_threads) as executor:
            ...
            for start_frame in range(0, total_frames, chunk_size):
                ...
                # 1. Запускаем экспорт фреймов для потока (ожидаем выполнения)
                extracted_frames = extract_frames(start_frame, end_frame)
                
                # 2. Запускаем поток с полученными фреймами
                future = executor.submit(chunk_processing, extracted_frames, start_frame, end_frame)
                futures.append(future)
                
                # 3. Если потоков уже >= max_threads - ждём завершения ближайшего
                if len(futures) >= max_threads:
                    done, not_done = wait(futures, return_when=FIRST_COMPLETED)
                    for f in done:
                        f.result()  # если в каком-то потоке была ошибка - прерываем всю обработку
